...
type Raft struct {
	...
	lastHeartbeat time.Time // 上次收到“领导关怀”的时间
}
...
func (rf *Raft) ticker() {
	...
	for rf.killed() == false {
		...
		rf.mu.Lock()
		//f or c, timeout, select !
		if time.Since(rf.lastHeartbeat) > time.Duration(ms)*time.Millisecond && rf.state != StateLeader {
			//start election
			go rf.startElection()
		}
		rf.mu.Unlock()
...
func (rf *Raft) startElection() {
	...
	rf.currentTerm++
	rf.votedFor = rf.me
	rf.lastHeartbeat = time.Now() // 重置一下时间，防止本轮没结束又开启一轮
